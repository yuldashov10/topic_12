...
> Важно: Запомните, множество неупорядоченное хранилище и хранит только уникальные элементы, значит проверяется не
...
> Запомните: запись через `{}` создаёт не множество, а пустой словарь (изучим в следующей теме).
...
`set.discard()` - безопасно удаляет элемент.
...
`set.union()` - объединяет множества.
...
`set.intersection()` - возвращает пересечение множеств.
...
`set.difference()` - возвращает разность множеств.

Возвращает новое множество из элементов, присутствующих в первом, но отсутствующих во втором.
...
`set.symmetric_difference()` - симметрическая разность множеств.
...
Возвращает `True`, если все элементы текущего множества содержатся в другом, в противном случае `False`.
...
Возвращает `True`, если текущее множество содержит все элементы другого, в противном случае `False`.
